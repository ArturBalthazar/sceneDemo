class CustomLogic {
  ...
  async attach(self, ctx) {
    ...
    // ✅ Assign AO maps after everything is initialized
    this._assignAOMaps();
  ...
  _assignAOMaps() {
    ...
    this._scene.meshes.forEach(mesh => {
      if (!(mesh instanceof BABYLONNS.Mesh)) return;

      const material = mesh.material;
      if (material && material.ambientTexture) {
        // Force AO map to use second UV set
        material.ambientTexture.coordinatesIndex = 1;
        console.log(`✅ AO map assigned -> Mesh: ${mesh.name}, Material: ${material.name}`);
      } else {
        console.log(`⚠️ No AO map found -> Mesh: ${mesh.name}`);
      }
    });
  }
